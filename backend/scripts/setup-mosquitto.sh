...
#
# Установщик MQTT-брокера Bellwake
#
# Поддержка:
#   Debian / Ubuntu
#   Mosquitto 2.0+
...
if ! command -v apt-get >/dev/null 2>&1; then
	die "Этот установщик поддерживает только Debian/Ubuntu (apt-get)."
fi
...
echo "==> Поиск модуля Dynamic Security..."
...
if [[ ! -f "${MQTT_KEY_FILE}" ]]; then
	die "Закрытый ключ TLS не найден:
...
# учётные данные должны уже присутствовать в .env.
#
...
#
# Заодно сохраняем значения по умолчанию, если их ещё не было.
#
...
#
# MQTT-брокер Bellwake
#
# Этот файл создан скриптом:
#
...
#
# Dynamic Security должен применяться ко всем обработчикам подключений.
#
...
#
# Постоянные сессии и сообщения для клиентов не в сети
#
# Пакет Debian/Ubuntu уже включает постоянное хранение и задаёт:
#
#   persistence true
...
#
# Сохраняем состояние брокера раз в минуту.
#

autosave_interval 60

#
# Удаляем заброшенные постоянные сессии по истечении этого срока.
#

persistent_client_expiration ${MQTT_SESSION_EXPIRATION}

#
# Максимальное число сообщений QoS 1/2 в очереди клиента не в сети.
#
# Сообщения Bellwake намеренно малы, поэтому 1000 — большой запас
# для обычного потока уведомлений.
#

max_queued_messages 1000

#
# Локальный обработчик административных подключений.
#
# Доступен только с самого сервера Bellwake.
#
...
# Сам установщик намеренно НЕ включает сетевой экран.
#
